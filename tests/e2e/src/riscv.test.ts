...
import {
  admin, adminClient, genRandomAccount, get_balance, transfer,
} from './common/utils';

const riscvService = new RISCVService(adminClient, admin);
...
async function check_deploy_auth(address: Address, is_exist: boolean) {
  ...
  const { addresses } = res0.succeedData;
  if (is_exist) {
...
describe('riscv service', () => {
  test('test_deploy_auth', async () => {
    ...
    const newService = new RISCVService(adminClient, account);
